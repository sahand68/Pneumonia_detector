...
model.compile(optimizer=tf.keras.optimizers.Adam(lr=.01),loss=tf.keras.losses.binary_crossentropy,metrics=['accuracy','sparse_categorical_accuracy'])
...
checkpoint = tf.keras.callbacks.ModelCheckpoint('model', monitor='val_loss', verbose=1, 
...
history = model.fit(train_gen, validation_data=valid_gen, epochs=45, shuffle=True, verbose=1,callbacks=callbacks_list,
                         workers=6)
